...
from layers import ConvSequence, WaveNetLayer
...
#       1[NOT DONE]. add Wavenet layer into the TT2 model
#       2[NOT DONE]. write decoder function to decoder text into audio msg
#       3[NOT DONE]. train model, save weights and implement TTS2 model into bot core
#   }
#   "after comit": {
#       1[DONE]. add Wavenet layer into the TT2 model
#       2[DONE]. write decoder function to decoder text into audio msg
#       3[DONE]. train model, save weights and implement TTS2 model into bot core
#   }
#}
...
class Tocotron2:
    ...
    def _build_decoder_(self):
        ...
        stop_condition_output = Dense(units=1, activation=decoder_params["dense_params"]["activation"], name="DecoderFirstOutputLayer")(dense_layer)
        ...
        self.decoder = Model(inputs=[input_layer, encoder_output_layer], outputs=[stop_condition_output, mel_sequence_output])
    ...
    def _build_model_(self):
        ...
        for layer_n in range(wavenet_params["layers_n"]):
            ...
            if layer_n == 0:
                wavenet_layer = WaveNetLayer(filters=wavenet_params["filters"],
                                            ...
                                            max_seq_len=wavenet_params["max_seq_len"])(decoder_forward[1])
        ...
        self.model = Model(inputs=[encoder_input_layer, decoder_input_layer], 
                           outputs=[decoder_forward[0], wavenet_layer])
        self.model.compile(loss=["mse", "categorical_crossentropy"], optimizer="rmsprop")
    ...
    def train_model(self, train_texts_sequences, train_voice_sequences, epochs, batch_size):
        ...
        self.model.fit([train_texts_sequences, train_voice_sequences], epochs=epochs, batch_size=batch_size)
        self.model.save_weigths(filepath=weights_path)
    
    def decode_sequence(self, text_sequences):
